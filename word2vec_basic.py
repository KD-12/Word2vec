...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  for i in range(batch_size):
      batchword_list.append(data[data_index])
  ...
  skipgram_temp=skip_window

  for i in range(int(batch_size / num_skips)):
      for _ in range(num_skips):

          batch_ind.append(skipgram_temp)
  ...
  for i in batch_ind:
      ...
      if i>0:
          for j in list2[k]:
              ...
              if j in temp and j!=i and flag:

                  flag=False
                  batch[temp1]=batchword_list[i]
                  labels[temp1]=batchword_list[j]



  data_index=data_index-window_size
  return batch, labels
...
if __name__ == '__main__':
  ...
  #batch_size = 512     #increasing batch batch_size
  batch_size = 128
  ...
  #max_num_steps   = 300001 #increased max_num_steps
